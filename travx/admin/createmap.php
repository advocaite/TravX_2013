...
function RandMap(){	
	...
	for($i=(0-$max_x);$i<=$max_x;$i++){
	  for($j=(0-$max_y);$j<=$max_y;$j++){
		...
		if($i < 0 && $j > 0){ $zid = 2; }
		if($i < 0 && $j < 0){ $zid = 1; }
		if($i > 0 && $j < 0){ $zid = 4; $zid2 = 5; }
		if($i > 0 && $j > 0){ $zid = 3; }
	
		$ii = preg_replace("/[^0-9]/","",$i);
		$jj = preg_replace("/[^0-9]/","",$j);
		if($ii <= 100 && $jj <= 100){ $register = true; }
/*		if($zid == 1){ if($i >= -100 && $j >= -100){ $register = true; } }
		if($zid == 2){ if($i >= -100 && $j <= 100){ $register = true; } }
		if($zid == 3){ if($i <= 100 && $j <= 100){ $register = true; } }
		if($zid == 4){ if($i >= -100 && $j <= 100){ $register = true; } }
		if($zid == 5){ if($i >= -100 && $j <= 100){ $register = true; } }*/
		
		if($kind < 7 && $register == true){
			$sql = "INSERT INTO wg_registration_village_list (village_id, zone_id, registed) VALUES ('".$v_id."','".$zid."','0')";
			$db->setQuery($sql);
			$db->query();
			if($zid2 == 5){
				$sql = "INSERT INTO wg_registration_village_list (village_id, zone_id, registed) VALUES ('".$v_id."','".$zid2."','0')";
				$db->setQuery($sql);
				$db->query();
			}
			if(mysql_error()){
