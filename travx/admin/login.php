...
$parse['message'] = NULL;
$parse['my_ip'] = $_SERVER['REMOTE_ADDR'];
...
if (count($_POST) <> 0 && $_POST['check'] == $_SESSION['security_adminlogin'])
{
    ...
    {
        returnMsg('Error:  The Validation code is wrong.');
    }

    if (empty($_POST['username']) || empty($_POST['password']))
    {
        returnMsg("Error: Please enter username and password.");
    }
    else
    {
        ...
        if ($wg_admin) // Login successfull
        {
            ...
            if ($st_forward != '')
            {
                returnMsg('{"msg":"Security error.","referer":"' . $st_forward . '"}');
                header("Location: $st_forward");
            }
            else
            {
                returnMsg('{"msg":"Security error","referer":"' . 'index.php' . '"}');
                header("Location: index.php");
            }
        }
        else
        {
            returnMsg("Error:  Login failed. System error");
        }
    }
}
