...
require_once('includes/wordFilter.php');
	...
	if(!empty($_GET['id']) && is_numeric($_GET['id']))
	{
		...
		{			
			if(count($_POST)<>0)
			{
				...
				if($_GET['act']=='answer')
				{
					$select = mysql_fetch_array(mysql_query("SELECT last_message FROM wg_users WHERE id='".$user['id']."' "));
					if($_POST['value']==$_SESSION['view_message'])
					{	
						if($select[0] > time() && $user['username'] != ''){
//						echo time()."<br/>".$select[0];
							$tpl='messages_spam';
							...
							$topic=$db->getEscaped(htmlspecialchars($_POST['topic']));
							$content=substr($db->getEscaped(htmlspecialchars($_POST['content'])),0,5000);
							$arr = array("travonline.net","tauturia.lt");
							$content = str_replace($arr,"localhost",$content);
			...
			(".$user['id'].",0,".$to_id.",'".date("Y-m-d H:i:s")."',0,'".$topic."','".$content."'),(".$to_id.",".$user['id'].",0,'".date("Y-m-d H:i:s")."',0,'".$topic."','".$content."')";
							...
							$db->query();	
							$antispam = time()+30;
							$sql="UPDATE wg_users SET last_message = '$antispam' WHERE id='".$user['id']."'";
							$db->setQuery($sql);
							$db->query();
				...
				$parse['topic']=''.$lang['33'].' '.stripslashes($query->subject).'';
				$parse['content']="\r\n\r\n___________________\r\n".GetNameByID($query->from_id)." ".$lang['wrote'].": \r\n".stripslashes(substr($query->content,0,3000))."";
				if($query->from_id==0)
				{
					$parse['content']='';
					$parse['topic']='';
			...
			else
			{
				...
				$parse['content']=stripslashes($query->content);
