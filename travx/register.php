...
if($_POST && $_POST['register'] == true){
		...
		if (!$resp->is_valid) { // Incorrect
			...
			if($check != FALSE){
				...
				$empty = true;
				$parse['error'] = "<font color='red'><b>NOTE!</b></font> Password is too short! (Minimum 6 characters lenght)";
			...
			if($empty == false){
				...
				$parse['suc'] = "<font color='green'><b>REGISTERED! <a href='login.php'>Click here to login</a></b></font><br/><br/>";
				$parse['error'] = "";
				$_POST = false;
			}
		}
...
$page =parsetemplate(gettemplate('register_new'), $parse);
