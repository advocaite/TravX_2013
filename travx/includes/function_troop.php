...
function Train($building, $status_type, $maxPalaceTrain=null){
	...
	}elseif($building->type_id==38){ // Trapper
		$maxTrapper = GetMaxTrapperTrain($building);
		if($maxTrapper['available'] < 1 || $maxTrapper['dunno'] < 1){
			$parse['no_troop']=$parse['khong_the_tao_them_tho'];
			$parse['list_troops']=parsetemplate(gettemplate("train_table_null"), $parse)."<br>";
		}else{
			$parse["list_troops"]=ShowListTroops($building, 0,$maxTrapper);
		}
...
function ShowPalaceCP($building){
	...
	if(count($_POST)<>0 && $_POST['input_diemDV'] >0)
	{
		...
		{
			$parse['error'] = "Score greater than the amount of population you have.";
		}
		else
		{
			...
			if(!$check)
			{
				$parse['error'] = "You do not have an alliance or coalition.";
			}
			else
			{
...
function ShowListTroops($building, $maxPalaceTrain=null, $trapper=null){
	global $lang, $wg_village;
	$parse=$lang;
	$result="";
	$i=0;
	...
	$listTroops=ListTroops($building, $trapper);
...
function ResearchTroop($troop_id, $listTroop){
	...
	if($wg_village->rs1>=$listTroop[$troop_id]->rsrs1 && $wg_village->rs2>=$listTroop[$troop_id]->rsrs2 && $wg_village->rs3>=$listTroop[$troop_id]->rsrs3 && $wg_village->rs4>=$listTroop[$troop_id]->rsrs4){
		...
		die("Hacking form reearch troop");
	}	
}
...
function SumTroops($name, $rs1, $rs2, $rs3, $rs4){
	...
	if($troop && $troop->rs1>0 && $troop->rs2>0 && $troop->rs3>0 && $troop->rs4>0){
		$sum=min(variantidiv($rs1, $troop->rs1), variantidiv($rs2, $troop->rs2), variantidiv($rs3, $troop->rs3), variantidiv($rs4, $troop->rs4));
		$result["id"]=$troop->id;
		$result["name"]=$name;
...
function GetMaxTrapperTrain($building){
	global $nationTroopList, $wg_village;
	
	## Define trap names, ids
	$soldierName = get_soldier($wg_village->nation_id);
	$soldierName = $soldierName.'12';
	$troopId = $nationTroopList[$wg_village->nation_id]['type_name12'];
	
	## Calculate how much do we have
	$sumTraining=CheckTrainTroop($wg_village->id, $troopId);
	
	## Calculate how much to let train per level
	$sumTroopTrained=$sumThoTrained + $building->level * 10;
	
	## Calculate total
	$sumTroop = $sumTraining + $sumTroopTrained;
	$speed = 300;
	$how = $building->level * 10 * $speed;
	if($sumTroop < $how){
		$sumTroopAvailable = $how - $sumTroopTrained;
	}else{
		$sumTroopAvailable = 0;
	}
	$result['available']=$sumTroopAvailable;
	$result['dunno']=intval($sumTroopAvailable/$how);
	return $result;
}
/**
 * @author Le Va Tu
...
function GetMaxTroopPalaceTrain($building){
    ...
    //traps in village
    $sumTrapsInVillage=GetSumPresentTroop(GetTroopIDByName($wg_village->nation_id, $soldierName3));
    ...
    //trapper
    $sumtrapstraining=CheckTrainTroop($wg_village->id, $troopId3);
    ...
    $sumTrapptrained=$sumTrapsTrained;
...
function SendTroops($building){
	...
	if(isset($_POST['vn'])){
		...
		if($_POST['vn']){//echo 'AAAAAAAAAAAAA';
			// Check if village exists or not
		...
		if($villageDefend->kind_id<7){
			...
			$userID_defend = GetPlayerID($villageDefend->id);
				if((get_level_building(12,$villageDefend->id)>=10)&& (check_iswar($userID_defend) < time()))
				{
					//cap nhat tinh trang chien tranh
					$time = time();
					$sql = "update wg_users set iswar = '".$time."' where id = ".$userID_defend;
					$db->setQuery($sql);
					$db->query();
				}
			//Check this corridor is in need of protection or does not:
			if(check_iswar($userID_defend) > time())
			{
			...
			if((get_level_building(12,$wg_village->id)>=10) && ($user['iswar'] < time()))
			{
				// War status update
				$time = time();
				$sql = "update wg_users set iswar = '".$time."' where id = ".$user['id'];
				mysql_query($sql);
			}
	...
	if(is_numeric($_POST['vdid'])){
		if(is_numeric($_POST['st']) && ($_POST['st']==1 || $_POST['st']==2 || $_POST['st']==3 || $_POST['st']==4)){
			...
			if($villageDefend->kind_id<7){
				...
				$userID_defend = GetPlayerID($villageDefend->id);
				if((check_iswar($userID_defend) > time()) || ($user['iswar'] > time()))
				{
...
function DisplaySendCatapult($villageDefend, $listTroop){
	...
	if($wg_village->user_id != $villageDefend->user_id && ($_POST['st'] == 3 || $_POST['st'] == 4))
	{
		...
		$mess = '<b>[!!!]</b> Population difference is: '.$unequal.'%.';
		if($unequal > 50)
		{
			$parse['mess'] = $mess.' If you attack him, you will loose '.($unequal - 50).'% of culture points! Be careful!.'; 	
		}
		else
		{
			$parse['mess'] = $mess;
	...
	{
		$unequal = (100 - (round(($villageDefend->workers*100/$wg_village->workers),0)));
		$mess = '<img src="images/warning1.gif"/> Population difference is: '.$unequal.'%.';
		if($unequal >50)
		{
			$parse['mess'] = $mess.' You will be charged '.($unequal - 50).'% of population.'; 	
		}
		else
		{
			$parse['mess'] = $mess;
...
/*	$mm = mysql_fetch_array(mysql_query($sql));
	print_r($mm);*/
	$db->setQuery($sql);
	return $db->loadResult();
}
...
function getChildVillage($villageId){
	...
	if(strlen(trim($objRes[0]->child_id))){// has child village
		$arrChild = explode("|", $objRes[0]->child_id);
	}
	else{
		return false;
	}
