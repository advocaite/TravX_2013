...
function includeLang($filename, $ext = '.mo')
{
	...
	if(!file_exists($ugamela_root_path . "language/" . $DEFAULT_LANG . '/' . $filename . $ext)){
...
function display($page, $title = '', $topnav = true, $rightnav = true, $metatags = '', $footer = true)
{
	...
	$usec = $sec[1];
	$sec = $sec[0];
	$parse['mytime'] = date("H:i:s");
	$parse['calc'] = ceil ( ( (double) $sec + (double) $usec - $GLOBALS['__scriptStart']) * 10);
		echo parsetemplate(gettemplate('footer'), $parse);
    }
...
function display_indzex($page, $title = '', $topnav = true, $metatags = '')
{
    global $link, $game_config, $debug, $user;
    echo "$page";
...
function display_index($page, $title = '', $topnav = true, $metatags = '')
{
    global $link, $game_config, $debug, $user;
    echo_head2($title, $metatags);
    echo "$page";
...
function echo_head($title = '', $metatags = '')
{
    ...
    if (check_user())
    {
        ...
        if ($wg_reports == 0 && $wg_messages == 0)
        {
            $parse['n5'] = 'm4';
        }
		elseif($wg_reports == 1 && $wg_messages == 0)
		{
			$parse['n5'] = 'm3';
		}
		elseif($wg_reports == 1 && $wg_messages == 1)
		{
			$parse['n5'] = 'm1';
		}
		elseif($wg_reports == 0 && $wg_messages == 1)
		{
			$parse['n5'] = 'm2';
		}
...
function echo_head2($title = '', $metatags = '')
{
    global $db, $user, $lang, $timeAgain, $game_config;
    includeLang ('simple_header');
    $parse                = $lang;
	...
	$parse['name']      = $lang['Login'];
	$parse['show_date'] = showDate();
	echo parsetemplate(gettemplate('simple_header3'), $parse);
