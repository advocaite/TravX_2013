...
function GetMerchantCapacity($village_id, $nation_id){
	...
	if($nation){
		$speed = 100;
		$capa=$nation->merchant_capacity * $speed;
...
function variantidiv($rs, $merchantCapacity){
	return $vl = $rs / $merchantCapacity;
/*	$value = $vl == 0 ? "1" : $vl;
	return $value;*/
}
...
function CheckMerchant($building, $rs){
	...
	$merchantRequire = $rs / $merchantCapacity;//variantidiv($rs, $merchantCapacity);
