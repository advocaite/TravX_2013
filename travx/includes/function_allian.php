...
function listVillageAndAllyMember($userid, $village)
{
	...
	$rnd = mt_rand(1,6);
	if($rnd == 1){
		$rnd_text = "Report server bugs to the admin!";
	}elseif($rnd == 2){
		$rnd_text = "You can find all server updates in our forum!";
	}elseif($rnd == 3){
		$rnd_text = "You can help us live by donating small amount of money.";
	}elseif($rnd == 4){
		$rnd_text = "Admin will answer all your questions!";
	}elseif($rnd == 5){
		$rnd_text = "Server creators are working hard to achieve the best community!";
	}else{
		$rnd_text = "Server speed: <b>300x</b>!";
	}
	$parse['random_text'] = $rnd_text;
...
{
    global $db;
    $sql = "SELECT name FROM wg_allies WHERE id = '".$allyId."' DESC LIMIT 1";
    $db->setQuery($sql);
    $query = null;
    $db->loadObject($query);
    return $query->name;
