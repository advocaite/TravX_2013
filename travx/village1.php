...
require_once('includes/func_build.php');
require_once('includes/func_convert.php');
require_once('includes/func_security.php');
require_once('includes/function_trade.php');
require_once('includes/function_status.php');
require_once('includes/function_attack.php');
require_once('includes/function_troop.php');
require_once('includes/wg_village_kinds.php');
...
if(!empty($_GET['a']) && is_numeric($_GET['a']) && isset($_SESSION['UpdateBuilding'.$_GET['a']]) )
{
	...
	$array=array();
	$get_a=$_GET['a'];
	$array=explode(",",$_SESSION['UpdateBuilding'.$get_a]);
...
$_SESSION['village_name'] = $wg_village->name;
if($wg_village->name=="NewName" || $wg_village->name=="")
{
	$parse['Name Village'] = $_SESSION['village_name'] = $lang['NewName'];
...
if($user['villages_id']==$village)
{
	...
	$_SESSION['is_captial'] = '<span class="c">['.$lang['capital'].']</span>';
}
...
$parse['map_typo'] = file_get_contents("includes/villages/kind".preg_replace("/[^0-9]/","",$image_kind).".txt");
$parse['fuckstyle'] = file_get_contents("includes/villages/kind".preg_replace("/[^0-9]/","",$image_kind)."css.txt");
...
#			echo "Level:".$ptu->level." -> Id:".$ptu->index." bID: $i<br/>";
...
#$parse['page_style_width'] = "<style>body{min-width:1300px;}</style>";
$parse['page_style_width'] ='';
